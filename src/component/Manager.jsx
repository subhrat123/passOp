    ...
    const savePasswords = async () => {
        ...
        if (passwords.id) {
            await fetch("http://localhost:3000/api/passwords", {
                ...
                body: JSON.stringify({ id: passwords.id })
            });
        }
            ...
            <div className='flex-col items-center'>
                ...
                <div className='flex-col container px-40 py-16 items-center mx-auto'>
                    ...
                    <div className='flex flex-col p-4 gap-8 items-center'>
                        ...
                        <button onClick={savePasswords} className='flex justify-center items-center gap-3 bg-green-400 px-7 py-2 rounded-full w-fit hover:bg-green-300'>
                        ...
                        {passwordArray.length !== 0 && <table className="table-auto w-full m-3 overflow-hidden rounded">
                            ...
                            <tbody className='bg-emerald-100'>
                                {passwordArray.map((item, i) => (
                                    <tr className='invert text-white' key={i}>
                                        <td className='text-center py-2 border border-white'>
                                            <div className='flex items-center justify-center gap-4'>
                                                <a href={item.site} target='_blank'>{item.site}</a>
                                                <img className='invert' onClick={() => copied(item.site)} src="svg/copy.svg" alt="copy" />
                                            </div>
                                        </td>
                                        <td className='text-center py-2 border border-white'>
                                            <div className='flex items-center justify-center gap-4'>
                                                {item.username}
                                                <img className='invert' onClick={() => copied(item.username)} src="svg/copy.svg" alt="copy" />
                                            </div>
                                        </td>
                                        <td className='text-center py-2 border border-white'>
                                            <div className='flex items-center justify-center gap-4'>
                                                <span>{'*'.repeat(item?.pass?.length || 0)}</span>
                                                <img className='invert' onClick={() => copied(item.pass)} src="svg/copy.svg" alt="copy" />
                                            </div>
                                        </td>
                                        <td className='text-center py-2 border border-white'>
                                            <div className='flex items-center justify-center gap-4'>
                                                <span onClick={() => { editPass(item.id) }}>
                                                    <lord-icon src="https://cdn.lordicon.com/oqaajvyl.json" trigger="hover"></lord-icon>
                                                </span>
                                                <span onClick={() => { deletePass(item.id) }}>
                                                    <lord-icon src="https://cdn.lordicon.com/vlnvqvew.json" trigger="hover" stroke="bold"></lord-icon>
                                                </span>
                                            </div>
                                        </td>
                                    </tr>
